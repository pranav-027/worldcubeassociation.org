...
import { Icon, Table, Segment, Pagination } from 'semantic-ui-react';
...
export default function BannedCompetitors({
  ...
  // Extract pagination metadata from API headers
  const totalEntries = headers ? parseInt(headers.get('total'), 10) : 0;
  const entriesPerPage = headers ? parseInt(headers.get('per-page'), 10) : 25;
    ...
    <>
      ...
      {totalPages > 1 && (
        <Pagination
          ...
          onPageChange={(e, { activePage: newPage }) => onPageChange(newPage)}
        />
      )}
    </>
  ) : (
